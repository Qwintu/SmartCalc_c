...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
  ...
  ui->dep_Periodicity_comboBox->addItems(
      {"choose", "monthly", "at the end" /*, "yearly" */});
...
void MainWindow::pushButton_calc_loan() {
  ...
  if (ui->ann_radioButton->isChecked()) {
    ...
    //        ui->schedule_plainTextEdit->appendPlainText("Month\tPayment\tBalance
    //        after payment"); for(size_t i = 0; i < total_months; i++){
    //            payment_monthly = ann_payment(balance_after_payment,
    //            interest_rate, total_months - i); balance_after_payment -=
    //            payment_monthly;
    //            ui->schedule_plainTextEdit->appendPlainText(QString::number(i
    //            + 1) + "\t" + QString::number(payment_monthly)+ "\t" +
    //            QString::number(balance_after_payment));
    //        }
    payment_monthly =
        ann_payment(balance_after_payment, interest_rate, total_months);
  ...
  if (ui->diff_radioButton->isChecked()) {
    ...
    for (int i = 0; i < total_months; i++) {
      percents_monthly =
          diff_payment_percents(balance_after_payment, interest_rate);
...
void MainWindow::pushButton_calc_dep() {
  ...
  last_date = today_date.addMonths(total_months);
  ...
  dep_total_percents = calc_deposit_persents(
      dep_amount, dep_interest_rate, dep_days, total_months, calculation_type);

  double dep_total_amount = dep_amount + dep_total_percents;
  double taxes = dep_total_percents * (dep_tax_rate / 100);
